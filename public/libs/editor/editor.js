    ...
    function (cmCfg, modeCfg) {
      ...
      function inlineNormal(stream, state) {
        ...
        if (ch === '*' || (ch === '_' && !ignoreUnderscore)) {
          if (state.strong === ch && stream.eat(ch) && stream.peek(ch)) {
            // Remove STRONG
            state.strong = false
            return t
          } else if (!state.strong && stream.eat(ch) && stream.peek(ch)) {
            // Add STRONG
            state.strong = ch
            return getType(state)
          } else if (state.em === ch) {
          ...
          if (state.strike === ch && stream.eat(ch)) {
            // Remove SRTIKE
            state.strike = false
            return t
          } else if (!state.strike && stream.eat(ch)) {
            // Add STRIKE
            state.strike = ch
            return getType(state)
          }
