...
const logger = useLogger(`[page:topic:${id}]`)

const { data: topic, fetch: getTopic } = useAPIData({
...
try {
  ...
  logger.error({ err }, 'get topic detail error')
}
...
async function handleTopicSubmit() {
  ...
  if (topic.value.title.length < 10) {
  ...
  toggleLoading()
  try {
    const { success, msg } = await $api.cnode.updateTopic({ topic_id: id, accesstoken: tokenCookie.value!, ...pick(topic.value, ['title', 'content', 'tab']) })
    if (success) {
      ElMessage.success({
        type: 'success',
        message: '更新话题成功',
      })
      await navigateTo({ path: '/', query: { tab: topic.value.tab } })
    }
    else {
    ...
    logger.error({ err }, 'update topic error')
    ElMessage.error({ type: 'error', message: '更新话题失败' })
  }
  finally {
    toggleLoading()
  }
