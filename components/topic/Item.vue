...
<template>
  <div class="relative grid grid-cols-[30px_70px_36px_1fr_80px] items-center border-t border-t-[#f0f0f0] bg-white p-2.5 text-sm first:border-t-0" :class="{ '!grid-cols-[30px_70px_1fr_50px]': !showTag }">
    <NuxtLink :to="`/user/${item.author?.loginname}`">
      <img
        class="block size-[30px] rounded-small"
    ...
    <span
      v-if="showTag"
      class="mr-1 rounded-small bg-[#e5e5e5] px-1 py-0.5 text-xs leading-3.5 text-text-2"
      :class="{ '!bg-primary !text-white': item.top || item.good }"
    >
      {{ item.top ? '置顶' : item.good ? '精华' : (item.tab && TAB_MAP[item.tab]?.name) || item.tab }}
    </span>
    <a
      class="truncate text-base leading-7.5 text-text-0 visited:text-[#888]"
      :href="`/topic/${item.id}`"
      :title="item.title"
    >
      {{ item.title }}
    ...
    <a
      class="text-right text-small text-text-2"
      :href="`/topic/${item.id}`"
    >
